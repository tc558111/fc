...
public class FTOFTdcApp extends FCApplication {
    ...
    public void updateCanvas(DetectorDescriptor dd) {
        ...
        H2D h2c = ftofPix[ilm].strips.hmap2.get("H2_t_Hist").get(is,0,0); h2c.setYTitle("Sector "+is+" PMT")       ; h2c.setXTitle("TDIF PMT TDC");
        canvas.cd(0); canvas.getPad().setAxisRange(1330.,1370.,1.,nstr+1) ; canvas.setLogZ(); canvas.draw(h2a); 
        canvas.cd(1); canvas.getPad().setAxisRange(1330.,1370.,1.,nstr+1) ; canvas.setLogZ(); canvas.draw(h2b); 
        canvas.cd(2); canvas.getPad().setAxisRange(-50.,50.,1.,nstr+1)    ; canvas.setLogZ(); canvas.draw(h2c); 
        ...
        h1c = ftofPix[ilm].strips.hmap2.get("H2_t_Hist").get(is,0,0).projectionY(); h1c.setXTitle("Sector "+is+" PMT" );       h1c.setFillColor(col1);  
        ...
        h1a = ftofPix[ilm].strips.hmap2.get("H2_t_Hist").get(is,1,0).sliceY(ics); h1a.setXTitle("Sector "+is+" Left PMT TDC");   h1a.setFillColor(col0);  
        h1b = ftofPix[ilm].strips.hmap2.get("H2_t_Hist").get(is,2,0).sliceY(ics); h1b.setXTitle("Sector "+is+" Right PMT TDC" ); h1b.setFillColor(col0);  
        h1c = ftofPix[ilm].strips.hmap2.get("H2_t_Hist").get(is,0,0).sliceY(ics); h1c.setXTitle("Sector "+is+" PMT TDIF" );     h1c.setFillColor(col2);  
