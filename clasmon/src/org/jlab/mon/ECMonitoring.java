...
public class ECMonitoring extends DetectorMonitoring {
	...
	public void drawComponent(int sector, int layer, int component, EmbeddedCanvas canvas) {
		...
		if (inProcess==1) {
	     ...
	     canvas.divide(3,3);
	     l=1;canvas.cd(l-1); u = ECAL_ADC.get(l).projectionY(); u.setXTitle("U STRIPS"); u.setFillColor(col0); canvas.draw(u);
	     l=2;canvas.cd(l-1); v = ECAL_ADC.get(l).projectionY(); v.setXTitle("V STRIPS"); v.setFillColor(col0); canvas.draw(v);
	     l=3;canvas.cd(l-1); w = ECAL_ADC.get(l).projectionY(); w.setXTitle("W STRIPS"); w.setFillColor(col0); canvas.draw(w);
	     if (layer<4) {
	    	 l=layer;canvas.cd(l-1); h = ECAL_ADC.get(l).projectionY(); h.setFillColor(col1); canvas.draw(h);
	         H1D copy = h.histClone("Copy");
	         copy.reset() ; copy.setBinContent(strip, h.getBinContent(strip));
	         copy.setFillColor(col2); canvas.draw(copy);
	     }
	     if (layer==7) {
	     ...
	     if (layer<4){
	     if(layer!=1) {l=1;canvas.cd(l+2); u = ECAL_ADC.get(l).sliceY(22);    u.setXTitle(alab[l-1]); u.setFillColor(col0); canvas.draw(u);}
	     if(layer!=2) {l=2;canvas.cd(l+2); v = ECAL_ADC.get(l).sliceY(22);    v.setXTitle(alab[l-1]); v.setFillColor(col0); canvas.draw(v);}
	     if(layer!=3) {l=3;canvas.cd(l+2); w = ECAL_ADC.get(l).sliceY(22);    w.setXTitle(alab[l-1]); w.setFillColor(col0); canvas.draw(w);}
	     l=layer;          canvas.cd(l+2); h = ECAL_ADC.get(l).sliceY(strip); h.setXTitle(alab[l-1]); h.setFillColor(col2); canvas.draw(h);
		 if(layer!=1) {l=1;canvas.cd(l+5); u = ECAL_TDC.get(l).sliceY(22);    u.setXTitle(tlab[l-1]); u.setFillColor(col0); canvas.draw(u);}
		 if(layer!=2) {l=2;canvas.cd(l+5); v = ECAL_TDC.get(l).sliceY(22);    v.setXTitle(tlab[l-1]); v.setFillColor(col0); canvas.draw(v);}
		 if(layer!=3) {l=3;canvas.cd(l+5); w = ECAL_TDC.get(l).sliceY(22);    w.setXTitle(tlab[l-1]); w.setFillColor(col0); canvas.draw(w);}
		 l=layer;          canvas.cd(l+5); h = ECAL_TDC.get(l).sliceY(strip); h.setXTitle(tlab[l-1]); h.setFillColor(col2); canvas.draw(h);
	     }
	     
	     if (layer==7){
	    	 for(int il=0;il<3;il++) {
	    		 canvas.cd(il+3) ; h = ECAL_ADC.get(il+1).sliceY(pixmap[il][pixel]-1); h.setXTitle(alab[il]); h.setFillColor(col2); canvas.draw(h);
	    		 canvas.cd(il+6) ; h = ECAL_TDC.get(il+1).sliceY(pixmap[il][pixel]-1); h.setXTitle(tlab[il]); h.setFillColor(col2); canvas.draw(h);
	    	 }
		 }
	     if (layer>7&&layer<12){
	    	 for(int il=0;il<3;il++) {
	    		 canvas.cd(il+3) ; h = ECAL_ADC_PIX.get(il+1).sliceY(pixmap[il][pixel]-1); h.setXTitle(alab[il]); h.setFillColor(col2); canvas.draw(h);
	    		 canvas.cd(il+6) ; h = ECAL_APIX.get(il+1).sliceX(pixel)                 ; h.setXTitle(alab[il]); h.setFillColor(col2); canvas.draw(h);
	    	 }
		 }
	     if (layer>11&&layer<16){
	    	 for(int il=0;il<3;il++) {
	    		 canvas.cd(il+3) ; h = ECAL_TDC_PIX.get(il+1).sliceY(pixmap[il][pixel]-1); h.setXTitle(tlab[il]); h.setFillColor(col2); canvas.draw(h);
