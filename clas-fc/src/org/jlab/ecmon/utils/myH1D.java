...
import org.jlab.data.histogram.Axis;
 ...
 * @version 062614
 */
public class myH1D implements EvioWritableTree,IDataSet {
    ...
    public void divide(myH1D hist1, myH1D hist2) {
        if(hist1.getAxis().getNBins() !=this.getAxis().getNBins())
        {
            System.out.println("ERROR: inconsistent bins in histograms");
            return;
        }
        ...
        {
            result.set(this.getBinContent(i), this.getBinError(i));
            hdiv.set(hist1.getBinContent(i), hist1.getBinError(i));
            result.divide(hdiv);
            hist2.setBinContent(i, result.number());
            hist2.setBinError(i, result.error());
        }
    }
