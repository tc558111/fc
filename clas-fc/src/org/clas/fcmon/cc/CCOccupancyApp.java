...
public class CCOccupancyApp extends FCApplication {
    ...
    public void updateCanvas(DetectorDescriptor dd) {
        ...
        H2D h2a = ccPix.strips.hmap2.get("H2_CCa_Hist").get(is+1,1,0); h2a.setYTitle(otab[0]+"PMTs") ; h2a.setXTitle(otab[0]+"PMT"+lab4[0]);
        H2D h2b = ccPix.strips.hmap2.get("H2_CCa_Hist").get(is+1,2,0); h2b.setYTitle(otab[1]+"PMTs") ; h2b.setXTitle(otab[1]+"PMT"+lab4[0]);
        ...
        for(int il=1;il<3;il++){
            ...
            canvas.cd(il+1); h1 = ccPix.strips.hmap2.get("H2_CCa_Hist").get(is+1,il,0).projectionY(); h1.setXTitle(xlab); h1.setFillColor(col0); canvas.draw(h1);
            }   
        
        canvas.cd(lr+1); h1 = ccPix.strips.hmap2.get("H2_CCa_Hist").get(is+1,lr,0).projectionY(); h1.setFillColor(col1); canvas.draw(h1,"same");
        ...
        for(int il=1;il<3;il++) {
            ...
            if(lr!=il) {canvas.cd(il+3); h1 = ccPix.strips.hmap2.get("H2_CCa_Hist").get(is+1,il,0).sliceY(11); h1.setXTitle(alab); h1.setTitle(""); h1.setFillColor(col0); canvas.draw(h1,"S");}
        ...
        canvas.cd(lr+3); h1 = ccPix.strips.hmap2.get("H2_CCa_Hist").get(is+1,lr,0).sliceY(ic);h1.setXTitle(alab); h1.setTitle(""); h1.setFillColor(col2); canvas.draw(h1,"S");
