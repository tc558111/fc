...
void GetFiberLengthsV(double array[], const int size);

void fitV(int Pass = 0)
{
	...
	TCanvas *canfit[63];
	TCanvas *cangr[63];
    //TCanvas *canallatt = new TCanvas("vstrip","V - Attenuation");
    ...
    if(Pass == 0)
	inhist = (TH3F*)gDirectory->Get("HADVvsRSUvstrips200");
    else
	inhist = (TH3F*)gDirectory->Get("HADVvsRSUvstripsTEST");

    //output file
    TFile fitsfile("VFits.root",filetype.c_str());
	...
	GetFiberLengthsV(fiblen,63+15);

	ofstream centroids ("Vcentroid.txt");
	ofstream UPar_file ("Vparameters.txt");
    ofstream VAtten ("VAtten.txt"); // strip, I0, A, B
    ofstream VGains ("VGains.dat"); // x = 0 value
	Initialize(parfit,3);
	...
	for(wstrip = 62; wstrip > 0; --wstrip)
	{
        ...
        Initialize(y,68);
        Initialize(yE,68);
		sprintf(name,"vstrip fits: %02d",wstrip);
...
/*************** Fit based on pass number *************************/
                if(Pass == 0)
                {
                    ...
                    if(par[2] > parE[2] && par[3] > parE[3] && par[4] > parE[4])
                    {
                ...
                else if(Pass > 0)
                {
                    ...
                    {
                        centroids << wstrip << "  " << ustrip << "  " << 0.0 << "   " << 0.0 << endl;
                    }
                }//Pass > 0
/************************* end fit of histograms ***************/
        ...
        */
		...
		if(count > 4)
		{
			sprintf(canname,"vstrip: %d",wstrip);
            sprintf(graphname,"vstrip%02dpass%1d",wstrip,Pass);
...
*/                
            ...
            */
            ...
            // strip, I0, A, B
            VAtten << wstrip << "    " << parfit[0] << "    " << parfit[1] << "    " << parfit[2] << endl;
            VGains << wstrip << "    " << 650.0/(parfit[0] + parfit[2]) << endl;
...
*/
        ...
        else if(count == 4)
        {
           ...
           // canallatt->cd(ustrip);
            ...
            // strip, I0, A, B
            VAtten << wstrip << "    " << parfit[0] << "    " << parfit[1] << "    " << 0.0 << endl;
            VGains << wstrip << "    " << 650.0/parfit[0] << endl;
        ...
        else if(count == 3)
        {
           ...
           // canallatt->cd(ustrip);
            ...
            VAtten << wstrip << "    " << parfit[0] << "    " << 0.0 << "    " << 0.0 << endl;
        ...
        {
            VAtten << wstrip << "    " << 0.0 << "    " << 0.0 << "    " << 0.0 << endl;
    ...
    DrawChiSquare(gcount, xsquare,2,Pass);
    DrawChiSquare(fitcount, x2square,20,Pass);
...
void GetFiberLengthsV(double array[], const int size)
{
	...
	ifstream fib_file ("FiberLengthsV.txt");
	for(i = 0; i < size; ++i)
	{
		fib_file >> num;
		fib_file >> array[num];
